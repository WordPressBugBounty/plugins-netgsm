...
Ayrıca eğer API erişiminizde IP sınırlaması yaptıysanız ve sınırladığınız ip dışında gönderim sağlıyorsanız 30 hata kodunu alırsınız. API erişim izninizi veya IP sınırlamanızı , web arayüzümüzden; sağ üst köşede bulunan ayarlar> API işlemleri menüsunden kontrol edebilirsiniz.";
    ...
    /**
     * Verilen telefon numaralarını, tüm kullanıcıları belleğe yüklemeden tek bir
     * usermeta sorgusuyla WP_User nesnelerine eşler. Büyük üye sayısına sahip
     * sitelerde get_users(-1) + döngü yerine kullanılır (bellek/performans).
     *
     * @param array  $phones   Aranacak telefon numaraları
     * @param string $meta_key Telefonun tutulduğu usermeta anahtarı
     * @return array           [baştaki sıfırları atılmış numara => WP_User]
     */
    public function phonesToUserMap($phones, $meta_key = 'billing_phone')
    {
        global $wpdb;
        ...
        foreach ((array) $phones as $p) {
            ...
            // Kayıtlarda numara sıfırlı/sıfırsız tutulmuş olabilir; her iki biçimi de ara.
            $candidates[$p]       = true;
            $candidates[$n]       = true;
            $candidates['0' . $n] = true;
        }
        if (empty($candidates) || $meta_key === '') {
            return array();
        }

        $values       = array_keys($candidates);
        $placeholders = implode(',', array_fill(0, count($values), '%s'));
        $params       = array_merge(array($meta_key), $values);
        $rows         = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT user_id, meta_value FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value IN ($placeholders)",
                $params
            )
        );
        ...
        $map = array();
        foreach ($rows as $row) {
            $key = ltrim(trim((string) $row->meta_value), '0');
